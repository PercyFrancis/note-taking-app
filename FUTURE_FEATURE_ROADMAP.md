...
The original `drawing` type remains a separate bitmap **Legacy canvas cell** for compatibility. Existing cells are never automatically converted. New toolbar buttons, per-cell buttons, and `Alt + Enter` create Excalidraw cells; enabling **Legacy canvas tools** under **Settings → Editor** reveals separate legacy creation buttons. This preference and touch drawing now synchronize through the account settings system.
